...
|---|---|---|
| `plots/mirt_k{K}{tag}/` | `diagnostics/plot_mirt.py`, and `fit.py --plots` | no |
| `plots/canonical/` | `fit.py --preset canonical` | no |
| `plots/dashboard/` | `build_dashboard.py --png` / `--pdf` | no |
| `index.html` (repo root) | `diagnostics/build_dashboard.py` | **yes** |
...
```bash
python diagnostics/plot_mirt.py --forecast --trace \
...
```bash
python diagnostics/plot_mirt.py --folder results/ --dry-run
python diagnostics/plot_mirt.py --folder results/mirt_dropFrontierMathv1AlgoTune_floors_poolednoise
```
