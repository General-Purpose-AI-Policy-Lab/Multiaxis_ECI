...
| `docs/figures/` (repo root) | `docs/make_readme_figures.py`: the README's figures, the flagship's four forecast panels and its loadings grid, on its majority chains without the chain-group note | **yes** |

Every `figures/` folder holds the PNGs, with the interactive Plotly twins under `html/` and the French versions under `fr/` (same layout inside, plus `fr/svg/`, the vector twins of the French renders for a document to embed: no title or subtitle, since the document sets the caption). They are gitignored: regenerable from the traces, and one dashboard build writes hundreds of them. `index.html` is the tracked artifact and the thing you serve. It is self-contained, so a browser opens it with no server. The blog post's figures are tracked because the post is a deliverable rather than a render, but their interactive twins are not: ~4.7 MB each of inlined JS, rebuilt by `make_all.py`.
...
The post's figures are the same builders as the outputs at another scale: each builder takes a `FigureStyle` (`viz/style.py`), `DASHBOARD` for the cards and the per-fit folders, `POST` for a figure shared flat at about 2,000 pixels, `READABLE` (the dashboard's layout with larger type) for the forecast panels of the cards and the per-fit folders, which are read on their own. The post's scripts own only the flagship trace, the caches beside it, the fixed windows (2023 to 2030 for the trend, 2015 to 2035 for the crossings) and the hand-placed callouts of figure 1; the figure design lives in `viz/` and cannot drift between the post and the outputs.
...
| key | what it shows |
|---|---|
| `forecast_{k}_{axis}` | the trend panel (`frontier_trend_fig`, `standalone=True`, `READABLE` type), built to read without a caption: the axis title set left over the subtitle *Frontier forecast against human tiers*, the y caption naming the axis (*Ability: Fluid Intelligence & Mathematics*). It draws the measured cloud (no whiskers), the two-regime forecast of `analysis.regimes` (the reasoning models' line and 80% band projected from their first release in orange; the non-reasoning models' line and band in muted purple over their own span, the line alone carried on thin to today so the change of slope shows), the releases each line was fitted on in that regime's colour with their 80% whiskers (muted red for reasoning, muted purple for the others, ringed in white over the cloud), the human tiers as thin solid lines named in the right margin, and a today line labelled *today*. A legend in the bottom-right corner lists *Reasoning models (frontier)*, *Projected trend*, *Non-reasoning models (frontier)*, *Projected trend*, *Non-frontier AI models*. The x-axis stops at 2030, the projection's horizon. The blog post draws the same builder without `standalone` (no legend, dashed tier lines, the bottom and top tiers' intervals, its caption naming the series) |
