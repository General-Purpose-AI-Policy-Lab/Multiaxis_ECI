...
Les données sont éparses, la matrice sujets × benchmarks n'étant remplie qu'à 6 %.
Le modèle ne converge donc pas vers une réponse unique par lui-même. Deux a priori
d'ordre servent à l'identifier, un ordre strict sur les niveaux humains et une
...
- Les quatre axes qui émergent du modèle sont **Intelligence fluide**,
  **Connaissances et raisonnement scientifiques**, **Agentique** et
  **Questions-Réponses (saturées)**.
...
Chaque capacité estimée vient ainsi avec son incertitude.
...
Nous appelons l'indice reconstruit **ECI-H**, H pour les références humaines
(*human baselines*). Nos valeurs diffèrent de celles publiées par Epoch pour trois
raisons principales. D'abord, nous n'ajustons pas la même table, notre ensemble de
benchmarks étant plus large et écartant huit benchmarks faciles pour les humains.
Ensuite, Epoch publie une valeur par modèle, en retenant son meilleur score sur
chaque benchmark, alors que nous traitons chaque variante d'effort de raisonnement
comme un sujet à part entière, avec ses propres scores. Enfin, nous incluons les
références humaines.
...
*Figure 1 - Capacités des IA, niveaux humains et difficulté des benchmarks sur
...
L'intuition est la suivante. Chaque sujet possède quatre capacités qui forment son
profil de compétence, comme un élève peut être fort en algèbre et faible en
rédaction, et chaque benchmark pondère ces compétences selon l'importance qu'elles
ont pour lui. Comme les données seules ne suffisent pas à identifier le modèle, deux
hypothèses d'ordre viennent l'aider, un ordre strict sur les niveaux humains et une
amélioration attendue le long des versions d'une même famille de modèles. La
description détaillée du modèle et de ces hypothèses est donnée dans la section
Méthodologie, après les résultats.

## Résultats

Avec ces hypothèses en place, le modèle se fixe sur une réponse unique pour la
plupart des exécutions. Voyons ce qu'il a trouvé.

### Les axes

Nous nommons chaque axe d'après les benchmarks dont les vecteurs de pondérations lui
sont les plus colinéaires, c'est-à-dire les benchmarks qui sollicitent cette
compétence et presque rien d'autre.
...
- **Axe 3 — Agentique**, défini par GBAEval, le Remote Labor Index et SWE-Bench Pro,
  ...
  questions.
- **Axe 4 — Questions-Réponses (saturées)**, composé pour l'essentiel d'anciens
  benchmarks de questions-réponses largement saturés, OpenBookQA, ARC (AI2), BoolQ
  et similaires.

![Figure 2](figures/fr/loadings_axes_plotly_draft.png)
*Figure 2 - Les 20 benchmarks qui définissent le mieux chaque axe. Les barres
représentent les pondérations, ou loadings (médiane, intervalle à 95 %).*
...
*Figure 3 - Meilleurs modèles et les neuf niveaux humains sur chaque axe (médiane,
intervalle à 95 %).*
...
modèles. Sur les Questions-Réponses (saturées), les humains dominent également, mais
il s'agit plutôt d'un artefact de données. Les huit benchmarks qui définissent cet
axe le plus purement (part d'axe supérieure à un demi) n'ont plus été passés par des
modèles depuis mi-2024, la plupart y plafonnant déjà autour de 0,9, et aucun modèle
de frontière n'y a jamais été mesuré. L'avance humaine sur cet axe est donc une
...
frontière signalées). Nous prenons les modèles détenteurs du record pour chaque jeu
plausible de capacités produit par le modèle, et nous construisons une **enveloppe
des records** par tirage, c'est-à-dire le maximum courant de la frontière dans chaque
tirage du posterior. Nous la prolongeons ensuite à son rythme mesuré sur une fenêtre
de 1,5 an, pour voir quand elle atteint chaque niveau humain. Comme indiqué plus
haut, les positions des modèles récents sur l'axe Questions-Réponses (saturées)
proviennent de l'a priori plutôt que de capacités mesurées. Nous laissons cet axe
hors des prévisions. Les chiffres mis en avant ci-dessous correspondent au mode
majoritaire (6 exécutions sur 10, le mode minoritaire étant en annexe).
...
modèle. Sur WMDP Biologie, l'écart est de 0,605 contre 0,875, et sur GPQA Diamond de
0,812 contre 0,946. De fait, chaque modèle sur cet axe se place au-dessus des
références humaines depuis 2023. Cela en dit plus sur ce que ces benchmarks
récompensent, l'étendue du rappel de connaissances à travers tout un champ, que sur
la capacité à faire de la science. Le Généraliste qualifié est sous le niveau du
hasard sur GPQA Diamond, à 0,22, et même le doctorant du domaine n'obtient que 0,43
sur WMDP Chimie.

![Figure 4](figures/fr/forecast_trend_plotly_majority.png)
*Figure 4 - Tendance de la frontière par axe (chaînes majoritaires). Les points sont
...
*Figure 5 - Dates auxquelles la tendance extrapolée atteint chaque niveau humain
...
pente. Elle paraît raisonnable dans notre fenêtre d'observation, où la frontière ne
...
supplémentaires, et bien davantage sur l'axe Agentique. Le meilleur moyen de les
...
Chaque sujet *m* possède quatre capacités qui forment son profil de compétence.
Chaque benchmark *b* pondère ces compétences à travers quatre *loadings* positifs
(ses saturations factorielles), un par axe, qui disent combien chaque compétence
compte pour ce benchmark. Les loadings règlent aussi la finesse avec laquelle un
benchmark sépare ses sujets, ce que la littérature psychométrique appelle la
discrimination. Un benchmark aux loadings élevés distingue nettement les modèles
faibles des modèles forts, un benchmark aux loadings faibles ne réagit guère à la
compétence. La difficulté est la barre que la somme pondérée des compétences doit
franchir pour dépasser le score médian, et une courbe en S transforme le résultat en
un score entre 0 et 1. Il faut enfin tenir compte du hasard. Nous fixons à l'avance
le plancher de chance de chaque benchmark et faisons démarrer la courbe à ce plancher
plutôt qu'à 0, de sorte qu'un QCM à quatre options a un score plancher de 0,25 et non
de 0.
...
de pondérations expliquent les scores aussi bien les uns que les autres. Des
exécutions répétées retombaient donc sur des solutions différentes. Il a fallu
injecter dans le modèle davantage d'information a priori pour qu'il converge vers une
réponse unique.
...
priori. Un Expert du domaine y est au moins aussi bon qu'un Généraliste qualifié, et
un comité au moins aussi bon que chacun de ses membres, sur chaque compétence.
L'ordre ne dit rien de la taille des écarts entre niveaux. Là où aucun classement ne
s'impose, entre un Meilleur performeur et un comité d'experts par exemple, nous
n'imposons rien. Les deux niveaux lycéens rejoignent l'ordre par deux liens, l'Expert
du domaine étant au moins aussi bon que le Lycéen qualifié et le Meilleur performeur
au moins aussi bon que le Lycéen meilleur performeur.
...
sein d'une même lignée, les GPT successifs ou la série Claude Opus par exemple, on
peut s'attendre à ce que chaque nouvelle version améliore la précédente. Une version
peut régresser si les données le disent. Nous ne faisons que la pousser doucement
vers l'amélioration. Nous utilisons aussi le temps écoulé entre deux sorties pour
calibrer l'écart attendu. Le gain espéré croît avec l'intervalle, et un laboratoire
qui publie beaucoup de petites mises à jour n'est pas supposé gagner plus qu'un autre
qui publie une seule grosse version sur la même année. Quant aux variantes d'effort
de raisonnement, elles sont rattachées à leur version de base et nous ne les
ordonnons pas entre elles, un effort plus élevé pouvant conduire le modèle à trop
réfléchir.
...
activées, et les exécutions se séparent encore en deux. Ces hypothèses améliorent par
...
- **Faire passer les benchmarks saturés aux modèles de frontière actuels.** Cela
  permettrait de trancher l'axe Questions-Réponses (saturées) avec des données
  plutôt que de le laisser à l'état d'artefact, tout en remplissant la grille au
  passage.
...
- **Tester les prévisions dans les deux sens.** Ajuster le modèle sur un instantané
  ...
  Vers l'avant, consigner les prédictions faites ici et suivre leur réalisation.
...
L'écart entre le mode minoritaire et le mode majoritaire, moyenné sur les neuf
niveaux, s'établit à −2,71 sur l'Agentique, +1,58 sur les Questions-Réponses
(saturées), +0,71 sur l'Intelligence fluide et −0,04 sur les Connaissances et le
raisonnement scientifiques. Les deux groupes partagent un même système d'axes et
décrivent les scores aussi bien l'un que l'autre.

![Figure 12](figures/fr/human_modes_plotly.png)
*Figure 12 - Les niveaux humains, les six exécutions majoritaires (bleu) face aux
quatre autres (orange).*

![Figure 13](figures/fr/split_takers_agentic_plotly.png)
*Figure 13 - Les 18 modèles que les deux groupes d'exécutions placent différemment,
...
*Figure 14 - Dates auxquelles la tendance extrapolée atteint chaque niveau humain
...
Pour les chaînes majoritaires, les fenêtres à 95 % sont nettement plus larges sur
tous les axes, mais se referment au plus tard vers 2035. Pour les chaînes
minoritaires, l'axe Intelligence fluide garde des intervalles à 95 % relativement
resserrés. Partout ailleurs, les incertitudes deviennent très grandes.
...
calibré répartirait ces valeurs uniformément. Notre histogramme bombe au centre, les
scores observés tombant près du centre des intervalles prédictifs plus souvent qu'ils
ne le devraient. Les intervalles sont donc plus larges que nécessaire, et le modèle
plutôt conservateur.
