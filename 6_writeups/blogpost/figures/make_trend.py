...
fit's forecast. Axis identity is checked against `analysis.require_axis_titles` (the signatures in `axis_names.json`)
before the cache is written, so a reused cache is a checked one.

Usage:
    python 6_writeups/blogpost/figures/make_trend.py [--trace FILE] [--tag ""]
"""
...
def forecast(trace: Path, cached: bool = False,
    ...
    display frame first, exactly as in `make_crossover`.
    """
...
def main(trace: Path = TRACE, tag: str = "", out_dir: Path = HERE,
    ...
    out = out_dir / f"forecast_trend{tag}"
