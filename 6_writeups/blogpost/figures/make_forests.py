...
`make_timeline.py` / `make_loadings.py`. Reads the flagship
...
fit-level display frame first, exactly as in `make_crossover`. Axis
identity is checked against `analysis.require_axis_titles` (the signatures in `axis_names.json`) before any label is
applied.

The chain subset names the output: `forests_axes_humanmerge_majority`,
`_minority`, `_c<chains>`, or no suffix for the whole posterior.

Usage:
    python 6_writeups/blogpost/figures/make_forests.py [--trace FILE] [--tag _humanmerge]
                                                   [--chains 0,1,3,8 | all]
"""
...
def main(trace: Path = TRACE, tag: str = "_humanmerge", out_dir: Path = HERE,
    ...
    out = out_dir / f"forests_axes{tag}{chain_suffix(chains)}"
    save_html(fig, out)
    save_print(fig, out)
