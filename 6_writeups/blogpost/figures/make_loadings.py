...
type scale (the constants match `make_timeline.py`), reading the
...
Usage:
    python 6_writeups/blogpost/figures/make_loadings.py [--trace FILE] [--tag _humanmerge]
"""
...
def main(trace: Path = TRACE, tag: str = "_humanmerge", out_dir: Path = HERE) -> None:
    ...
    out = out_dir / f"loadings_axes{tag}"
    save_html(fig, out)
    save_print(fig, out)
