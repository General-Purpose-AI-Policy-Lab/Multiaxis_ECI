...
Usage:
    python 6_writeups/blogpost/figures/make_timeline.py [--results DIR] [--tag _humanmerge]

The tag names the FIT, not the drafting state: `_humanmerge` is the reference render
(canonical_humanmerge/, the human tier order every analysis passes) and `_nohumanorder`
its sensitivity twin on canonical/. Every post figure moved from the old `_draft` tag to
`_humanmerge` on 2026-09-15, and `_plotly` was dropped from the file names at the same
time; the module names still carry it.
"""
...
# canonical_humanmerge/ since 2026-09-15: --human-merge is the human tier order every
# analysis passes, so figure 1 reads the merged fit and the unordered canonical/ becomes
# the sensitivity render (make_french.py --only timeline_nohumanorder).
DEFAULT_RESULTS = config.RESULTS_DIR / "canonical_humanmerge"
...
def main(results: Path, tag: str, out_dir: Path = HERE) -> None:
    ...
    out = out_dir / f"eci_1d_timeline{tag}"
...
if __name__ == "__main__":
    ...
    p.add_argument("--tag", default="_humanmerge")
    args = p.parse_args()
