...
Runtime source: data/processed/benchmarks_merged.csv. This module does only the modeling-stage
...
`data/curated/benchmark_lower_bounds.csv`, read directly by the `--floors`
...
from config import DATA_DIR, ECI_EPS, LOW_OBS_THRESHOLD, ZERO_DIAG_THRESHOLD

# Subdirectory layout:
#   data/raw/        — reference ECI CSV (only used by --eci-data-only mode)
#   data/curated/    — user-maintained inputs (excluded list, human baselines)
#   data/processed/  — output of data/pipeline/pipeline.ipynb
RAW_DIR       = DATA_DIR / "raw"
CURATED_DIR   = DATA_DIR / "curated"
PROCESSED_DIR = DATA_DIR / "processed"

# Re-run data/pipeline/pipeline.ipynb to refresh this file.
...
def clip_scores_to_floors(data: "ECIData", floors: np.ndarray) -> "ECIData":
    ...
    The clips are APPLIED FROM `data/curated/benchmark_score_clips.csv` — the
    ...
    `diagnostics/audit_lower_bounds.py --write-clips` after a data or floor
    ...
    for i, key in enumerate(obs_key):
        ...
        if abs(data.scores[i] - r.original_score) > 1e-6:
            warnings.warn(
                ...
                f"Refresh with diagnostics/audit_lower_bounds.py --write-clips.",
                UserWarning)
            continue
    ...
    if stale:
        warnings.warn(
            ...
            f"diagnostics/audit_lower_bounds.py --write-clips.", UserWarning)
    ...
    if below.any():
        ...
        warnings.warn(
            ...
            f"diagnostics/audit_lower_bounds.py --write-clips and review.",
            UserWarning)
...
def load_eci_data(drop_low_obs_models: bool = False,
    ...
    `data/pipeline/pipeline.ipynb`, which writes the complete
    ...
    `excluded_benchmarks.txt` list, which is filtered out here at fit time):
      ...
      • fit_humans (default True) — concatenate `data/curated/human_baselines.csv`
      ...
      • fit_cyber (default False) — append `data/curated/cyber_benchmarks.csv`,
      ...
      • fit_simpleqa_original (default False) — append
        `data/curated/simpleqa_original/simpleqa_original.csv`, the original
    ...
    if eci_data_only:
        ...
        if not PROCESSED_FILE.exists():
            raise FileNotFoundError(
                ...
                "`data/pipeline/pipeline.ipynb` (Restart Kernel → Run All) "
        ...
        if collapse_effort_variants:
            from config import ANCHOR_HIGH, ANCHOR_LOW, SOTA_MODELS
    ...
    from config import SOTA_MODELS
