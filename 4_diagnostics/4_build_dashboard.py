...
Run:  python 4_diagnostics/4_build_dashboard.py
"""
...
modes_path = _load_script("4_diagnostics/diagnose_chains.py").modes_path
...
from multiaxis_eci.ppc import (  # noqa: E402
    compute_gof,
    posterior_predictive_mirt,
    posterior_predictive_mirt_nc,
)
from multiaxis_eci.viz import (  # noqa: E402
    assemble_dashboard,
    build_axis_figures,
    build_comparison,
    build_fit_figures,
...
def _add_fit(args):
    ...
    """
    _spec_of = _load_script("4_diagnostics/3_plot_mirt.py")._spec_of
...
def main():
    ...
    raw = pd.read_csv(PROCESSED_FILE)[SCORE_COLUMNS]
    ...
    # Raw-data section: no fit, no trace, no cache — the score view itself plus
    # the opt-in curated extra-benchmark file (SimpleQA original; same eight
    # columns), one legend-toggleable trace per benchmark.
    extra_csvs = [
