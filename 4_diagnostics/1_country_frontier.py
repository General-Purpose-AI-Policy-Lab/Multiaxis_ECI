...
Run: python 4_diagnostics/1_country_frontier.py [--open-only|--closed-only]
"""
...
from multiaxis_eci.analysis.forecast import (  # noqa: E402
    _to_date,
    _to_year,
    mirt_crossover_df,
...
def load_country_map() -> dict:
    """model_version -> ISO country code, from the pipeline's models table (0_input/models.csv)."""
    df = pd.read_csv(MODELS_FILE)
    return dict(zip(df["model_version"], df["country"].fillna("")))
