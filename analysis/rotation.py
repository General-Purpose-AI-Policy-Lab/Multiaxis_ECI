...
def nonneg_rotate(L: np.ndarray, pen: float = 50.0, restarts: int = 40,
                  seed: int = 0, max_iter: int = 8000):
    """Oblique rotation maximizing the varimax simple-structure criterion
    SUBJECT TO non-negative loadings — the contrast-free interpretation frame.

    Promax and geomin are sign-free: their target-chasing suppresses small
    loadings past zero, so even when the FITTED loadings are all non-negative
    (the `normal` prior) the rotated axes come out bipolar (a benchmark that is
    strong on one axis reads as negative on another). This criterion adds a
    hard penalty on negative rotated cells, so every axis stays a pure
    'good-at-these' bundle with no anti-loaded benchmarks. It only has a clean
    solution when the fitted subspace is itself non-negatively representable:
    on a signed fit the constraint collapses simple structure to ~0 (the
    subspace carries a genuine contrast that no non-negative basis can hold).

    Same contract as promax_rotate — returns (Tload, Ttheta, Phi) with
    L_rot = L @ Tload, theta_rot = theta @ Ttheta (predictions invariant),
    Phi = factor correlation. The transform Tload = M is optimized DIRECTLY
    (L_rot = L @ M), started from the identity — where L_rot = L is already
    non-negative (the `normal` prior), so the search begins feasible and stays
    well-conditioned. (An inverse parameterization L_rot = Lv @ T⁻¹ is not used:
    the scale-free criterion rewards near-singular T with a degenerate spike.)
    Phi = normalize(inv(MᵀM)) with unit-variance factors — identical to the
    promax convention (there M = R·Q ⇒ inv(MᵀM) = inv(QᵀQ)). Optimized by
    multi-start Nelder-Mead (the criterion is non-smooth at the non-negativity
    boundary); fixed seed + identity start make it deterministic, and on this
    data it is a single well-defined optimum (seed-invariant, matched-axis
    correlation 1.000 across restarts)."""
    ...
    def obj(x):
        ...
        if abs(np.linalg.det(M)) < 1e-6:
            return 1e6
        Lr = L @ M
        s = np.abs(Lr).max(axis=0); s[s == 0] = 1.0
        Ls = Lr / s                                             # per-column scale (orientation only)
        h = np.sqrt((Ls ** 2).sum(axis=1, keepdims=True)); h[h == 0] = 1.0
        Lk = Ls / h                                             # Kaiser row-normalize (standard varimax)
        simple = sum(np.var(Lk[:, k] ** 2) for k in range(K))   # varimax (maximize)
        neg = float(np.sum(np.minimum(Ls, 0.0) ** 2))           # non-negativity (penalize)
        return -simple + pen * neg

    rng = np.random.default_rng(seed)
    best = None
    for r in range(restarts):
        x0 = (np.eye(K).ravel() if r == 0
              else (np.eye(K) + 0.3 * rng.standard_normal((K, K))).ravel())
        res = minimize(obj, x0, method="Nelder-Mead",
                       options=dict(maxiter=max_iter, xatol=1e-7, fatol=1e-9))
        if best is None or res.fun < best.fun:
            best = res
    M = best.x.reshape(K, K)
    # Fix factor variances to 1 (promax convention): scale columns so that
    # diag(inv(MᵀM)) = 1, then Phi = inv(MᵀM) has unit diagonal.
    Ginv = np.linalg.inv(M.T @ M)
    M = M * np.sqrt(np.diag(Ginv))[None, :]
    Ginv = np.linalg.inv(M.T @ M)
    Phi = Ginv.copy()
    off = np.abs(Phi - np.eye(K))
    if off.max() > 0.98:
        raise ValueError(f"nonneg_rotate degenerate: |Phi off-diag| max {off.max():.3f} "
                         "(near-collinear axes — no clean non-negative rotation exists; "
                         "expected on a signed-prior subspace)")
    Tload = M
    Ttheta = np.linalg.inv(Tload).T
    rng2 = np.random.default_rng(0)
    th = rng2.standard_normal((5, K))
    assert np.allclose((L @ Tload) @ (th @ Ttheta).T, L @ th.T, atol=1e-8), \
        "nonneg rotation broke prediction invariance"
    return Tload, Ttheta, Phi
