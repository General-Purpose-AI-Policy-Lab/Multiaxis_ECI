...
Country map: data/curated/model_country.csv. Scope flags mirror fit.py's
--open-only / --closed-only. Both selection variants (informed-only and
SOTA-admitted) land in the CSV; figures show informed-only.

Run: python diagnostics/country_frontier.py [--open-only|--closed-only]
"""
...
from analysis.forecast import (_to_date, _to_year, mirt_crossover_df,  # noqa: E402
                               mirt_frontier_forecast)
from analysis.stats import _release_dates, capability_draws, eci_transform  # noqa: E402
from analysis.timelines import mirt_informed_mask  # noqa: E402
from data import PROCESSED_FILE, load_eci_data  # noqa: E402
from fit import open_only_drop_list  # noqa: E402
from persistence import load_trace, save_df  # noqa: E402
...
def main():
    ...
    ap.add_argument("--open-only", action="store_true",
                    help="use the --open-only data scope (needs "
                         "data/curated/benchmark_access.csv)")
