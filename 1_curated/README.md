...
| `sota_families.txt` | generated | `config.SOTA_FAMILIES` (timelines always show every effort of the release, the country frontier admits them as record candidates, the SOTA table of the canonical fit keeps the best effort; the list never decides what the fit sees) | no, regenerate with `python 1_curated/1_compute_sota.py` after a canonical fit: world frontier records of the last 24 months plus every family within 10 ECI of the best model of each organisation on that frontier, one family (base model and snapshot) per line, candidates with at least 4 observations |
