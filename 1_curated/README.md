...
| `sota_families.txt` | generated | `config.SOTA_FAMILIES` (plot exemptions and drop-filter protection for every effort of the release, the SOTA table of the canonical fit at the best effort) | no, regenerate with `python 1_curated/1_compute_sota.py` after a canonical fit: world frontier records of the last 24 months plus every family within 10 ECI of the best model of each organisation on that frontier, one family (base model and snapshot) per line, candidates with at least 4 observations |
