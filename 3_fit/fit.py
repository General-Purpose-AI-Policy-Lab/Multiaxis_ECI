...
Two modes:
  ...
  python 3_fit/fit.py --K 4 [--human-merge --lineage-prior --lineage-bm ...]
      ...
      5_outputs/<data generation>/mirt{tag}/ folders. Non-negative loadings and fixed-c 3PL
...
# The library lives in 2_model/; make it importable without `pip install -e .`.
...
from multiaxis_eci import (
    config,  # attribute access so --eci-data-only / --raw-c overrides propagate
)
...
from multiaxis_eci.persistence import (
    load_trace,
    save_df,
    save_json,
    save_pit,
...
def run_canonical(args) -> None:
    ...
    # config.RESULTS_DIR is 5_outputs/<data generation>/, so a fit on a newer
    # pipeline build never overwrites the published one.
    results_dir = config.RESULTS_DIR / scope_tag
    figures_dir = results_dir / config.FIGURES_DIRNAME
    ...
    save_df(D_df, results_dir / "benchmark_difficulty.csv")
    save_df(A_df, results_dir / "benchmark_discrimination.csv")
    ...
    # ── Plots ──────────────────────────────────────────────────────────────
    print("\n── Plots ────────────────────────────────────────────────────────")
    save_fig(hyperparams_fig(trace), "hyperparameters", figures_dir)
    save_fig(forest_fig(D_df, "D", "Per-benchmark difficulty (D)"),
             "benchmark_difficulty", figures_dir)
    save_fig(forest_fig(A_df, "A", "Per-benchmark discrimination (A, K=1 loading)"),
             "benchmark_discrimination", figures_dir)
    ...
    save_fig(sota_forest_fig(sota, x_col="ECI",
                             ...
                             anchors=anchors),
             "sota_models_eci", figures_dir)
    save_fig(sota_forest_fig(sota, x_col="C",
                             title="SOTA capability (C)",
                             xaxis_title="C"),
             "sota_models_capability", figures_dir)
    ...
    save_df(all_models, results_dir / "all_models_capability.csv")
    ...
    save_fig(all_models_forest_fig(ai_only, metric="C"),
             "all_models_capability", figures_dir)
    ...
    save_fig(all_models_forest_fig(ai_only_eci, anchors=anchors),
             "all_models_eci", figures_dir)

    tl = timeline_stats_df(trace, data, raw_df)
    human_names = set(humans["name"]) if len(humans) else set()
    save_df(tl[~tl["name"].isin(human_names)], results_dir / "timeline.csv")

    # The ECI-H timeline: the post's figure 1 (anchored scale, 80% intervals, the
    # bottom and top human tiers with their interval as a band), English by
    # default and its French render under figures/fr/.
    tl_eci = timeline_stats_df(trace, data, raw_df, metric="ECI", hdi_prob=TIMELINE_HDI_PROB)
    tl_eci = tl_eci[~tl_eci["name"].isin(human_names)]
    humans_eci = human_stats_df(trace, data, metric="ECI", hdi_prob=TIMELINE_HDI_PROB)
    save_df(tl_eci, results_dir / "timeline_eci.csv")
    save_df(humans_eci, results_dir / "human_groups_eci.csv")
    for lang, sub in (("en", figures_dir), ("fr", figures_dir / "fr")):
        save_fig(capability_timeline_fig(
                 ...
                 "capability_timeline", sub)

    save_fig(pit_hist_fig(gof.pit), "calibration_pit_histogram", figures_dir)
    save_fig(pit_ecdf_fig(gof.pit), "calibration_pit_ecdf", figures_dir)
    save_fig(density_overlay_fig(y_rep_flat, data.scores), "ppc_score_density", figures_dir)
    ...
    save_fig(pred_vs_obs_fig(data.scores, gof.y_pred_mean, hover),
             "ppc_predicted_vs_observed", figures_dir)
    save_fig(residuals_per_benchmark_fig(resid_df),
             "residuals_per_benchmark", figures_dir)

    print(f"\nDone. Results in {results_dir}, figures in {figures_dir}.")
...
def run_exploration(args, parser) -> None:
    ...
    if args.plots:
        ...
        plot_fit = load("4_diagnostics/3_plot_mirt.py").plot_fit
...
def main():
    ...
    parser.add_argument("--eci-data-only", action="store_true",
                        help="[canonical] fit 1_curated/eci_data.csv (the original "
    ...
    parser.add_argument("--simpleqa-original", action="store_true",
                        help="[exploration] append the original OpenAI SimpleQA "
                             "(1_curated/simpleqa_original/). A separate column "
    ...
    parser.add_argument("--no-floors", dest="floors", action="store_false",
                        help="[exploration] drop the fixed-c 3PL floors, which are "
                             ...
                             "the view's lower_bound column and never "
