...
sys.path.insert(0, str(Path(__file__).resolve().parent / "2_model"))
...
def run_canonical(args) -> None:
    ...
    # The data generation goes in the folder name (config.DATA_SUFFIX), so a fit on a
    # newer pipeline build never overwrites the published one.
    results_dir = config.RESULTS_DIR / (scope_tag + config.DATA_SUFFIX)
    plots_dir = config.PLOTS_DIR / (scope_tag + config.DATA_SUFFIX)
